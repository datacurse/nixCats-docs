...
function prepareDoc(doc: string) {
  ...
  const href = `/${slugs.map(slugify).join('/')}`
...
export function getDocsNav(): NavigationTree {
  ...
  return {
    Introduction: [
      pages['what-is-nixcats'],
      pages['feature-highlights'],
      pages['quick-start'],
    ],
    'Getting Started': [
      pages['installation-and-template-picker'],
      pages['first-build-and-run'],
      pages['faq'],
    ],
    'Core Concepts': [
      pages['category-system'],
      pages['builder-and-packages'],
      pages['settings-and-wraprc'],
    ],
    Guides: [
      pages['daily-workflow'],
      pages['lsp-and-mason-integration'],
      pages['lua-utils'],
      pages['overriding-packages'],
      pages['modules'],
      pages['custom-overlays'],
    ],
    Examples: [
      pages['templates'],
      pages['kickstart-nvim'],
      pages['lazyvim'],
    ],
    API: [
      pages['lua'],
